...
class ImageResize //extends \yii\db\ActiveRecord
{
    ...
    private function thumbnail_it()
    {
        if ($this->file_count > 0) {
            ...
            foreach ($this->thumbnail_size as $ThumbKey => $ThumbValue) {
                for ($x = 0; $x < $this->file_count; $x++) {
                    ...
                    if (is_uploaded_file($this->file_data['tmp_name'][$x])) {
                        ...
                        if ($this->random_file_name && !empty($this->unique_rnd_name)) {
                            ...
                            $this->new_file_name = $this->thumbnail_prefix . $ThumbValue . "_" . $this->unique_rnd_name[$x];
                        } else if ($this->random_file_name) {
                            $this->new_file_name = $this->thumbnail_prefix . $ThumbValue . "_" . uniqid() . $this->get_extension();
                        } else {
                            #$this->new_file_name = $this->thumbnail_prefix.$this->file_data['name'][$x];
                            $this->new_file_name = $this->thumbnail_prefix . $ThumbValue . "_" . $this->file_data['name'][$x];
                        }

                        $this->image_res = $this->get_image_resource();
                        $this->new_width = $this->thumbnail_size[$ThumbKey];
                        $this->new_height = $this->thumbnail_size[$ThumbKey];
