$(document).ready(function () {
    var optionsFloating = $('.over-header');
    var defaultOptionsTop = optionsFloating.offset().top;
    var defaultOptionsBottom = $('.mega-footer').offset().top - $(window).height();
    ...
    $('#web_by').hover(function () {
            $('#jobotic').animate({right: '80px'}, 600, function () {
                $('#web_by label').css('display', 'block');
            });
        },
        function () {
            $('#web_by label').css('display', 'none').stop(true, true)
            $('#jobotic').animate({right: '1900px'}, 1800, function () {
                $('#jobotic').css('right', '-100px').stop(true, true)
            })
        });
    $('#web_by').click(function () {
        window.location.href = 'http://www.steinlinconsulting.com';
    });
