            ...
            if ($model->validate()) {
                if ($PIN == $PhonePin) {
                    ...
                    if ($temp['StartTime'] > 0 && $temp['StartTime'] <= Yii::$app->params['timePinValidate']) {
                        ...
                        $model->phone_pin = '';
                        $show = false;
                        #START : Check Phone Number existed or not .
                        $PhoneExist = User::findByPhoneNumber($Id, $model->new_phone_no);
                        #echo count($PhoneExist);CommonHelper::pr($PhoneExist);exit;
                        if (count($PhoneExist) > 0) {
                            $temp['PhoneExist'] = true;
                        }
                        #END : Check Phone Number existed or not .
                    } else {
                        $temp['Error'] = 1;
                    }
                }
